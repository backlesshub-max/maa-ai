...
// ----- Knowledge base (UPDATED: loads ALL datasets automatically) -----
function buildKnowledge(){
  const ds = window.DATASETS || {};
  const funBuilt = (window.buildFunQA ? window.buildFunQA() : []);

  // Take every array inside window.DATASETS (climatology, assam, india, self, geography, romance, etc.)
  const datasetArrays = Object.values(ds).filter(v => Array.isArray(v));
  const all = datasetArrays.flat().concat(funBuilt || []);
